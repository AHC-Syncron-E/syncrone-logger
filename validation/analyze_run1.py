...
def _(mo):
    mo.md(r"""
    # Run 1 — SPONT PS — Waveform Fidelity Analysis
...
@app.cell
def _(wfr):
    _dtb_path, _rwb_path, _db_path = wfr.find_run_files(1)
...
def _(df_syncrone, mo, wfr):
    _fig = wfr.plot_raw_syncrone(df_syncrone, " — Run 1 (SPONT PS)")
...
def _(df_dtb, df_syncrone, wfr):
    # Run 1 alignment parameters (validated in exploratory analysis)
    alignment = wfr.align_signals(
        df_dtb,
        df_syncrone,
        asl_t_min=170.0,          # signature disconnect event at ~170 sec
        sync_t_naive_min=58.0,    # skip pre-overlap Syncron-E data
    )
    return (alignment,)
...
def _(fidelity, mo):
    ...
    _table_body = "\n".join(_rows)
    mo.md(f"""## Waveform Fidelity Results — Run 1 (SPONT PS)
...
the ventilator outlet.  Circuit compliance and tubing resistance create a
physical difference between these two measurement points, particularly during
rapid flow transitions.  NRMSE normalises this against the full signal dynamic
range, providing a scale-appropriate measure of recording fidelity.
    """)
    return
...
def _(alignment, mo, wfr):
    _fig = wfr.plot_overlay(alignment.df_aligned, title_suffix=" — Run 1 (SPONT PS)")
...
@app.cell
def _(alignment, mo, wfr):
    _fig = wfr.plot_error_series(alignment.df_aligned, title_suffix=" — Run 1")
...
@app.cell
def _(alignment, mo, wfr):
    _fig = wfr.plot_error_histograms(alignment.df_aligned, title_suffix=" — Run 1")
...
@app.cell
def _(alignment, mo, wfr):
    _fig = wfr.plot_bland_altman(alignment.df_aligned, title_suffix=" — Run 1")
